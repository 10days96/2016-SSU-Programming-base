...
int main()
{
	...
	while (true) {	// 반복적으로 수식 입력을 받기 위한 while문
		...
		printf("(Input) ");
		get_string(cInput);			// Enter가 눌릴 때까지 공백을 포함한 입력스트림의 모든 문자를 배열에 저장합니다.

		// 입력 받은 문자열을 strcmp로 비교. 각, 명령어에 대한 실행을 해줍니다.
		// 화면 지우기
			...
			if ((('a' <= cInput[0] && cInput[0] <= 'z') || ('A' <= cInput[0] && cInput[0] <= 'Z')) && 1 == strlen(cInput)){
				...
				if (!bIsError)
				{
					...
					while (iNumOperator)
					{
						...
						// strtod 함수를 이용해서 문자열을 double형으로 변환
						dOperandLeft = convert_to_double(cInputOperand[iOperatorLocation]);
						dOperandRight = convert_to_double(cInputOperand[iOperatorLocation + 1]);
						switch (cOperator)
						{
						case '+':
...
{
	FILE *tmp1, *tmp2;
	tmp1 = fopen("tmp.out", "w");
	fprintf(tmp1, "%lf", dNum);
	fclose(tmp1);
	tmp2 = fopen("tmp.out", "r");
	fscanf(tmp2, "%s", cNumber);
	fclose(tmp2);
	return ERRCODE_OK;
...
double convert_to_double(char string[])
{
	FILE *tmp1;
	double result;
	tmp1 = fopen("tmp1.out", "w");
	fprintf(tmp1, "%s", string);
	fclose(tmp1);
	tmp1 = fopen("tmp1.out", "r");
	fscanf(tmp1, "%lf", &result);
	fclose(tmp1);
	return result;
}
